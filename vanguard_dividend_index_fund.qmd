...
library(lubridate)
library(pracma)
library(janitor) # clean  column name abb cde to abc_cde and all lower case
```
```{r}
rm(list=ls())
...
```{r}
funds |> select(close,short_mean,medium_mean,long_mean) |> 
  summary() 
```

```{r}

funds |> filter(date >="2024-01-01") |> ggplot() + geom_line(aes(x=date,y=close,col="Close"),linewidth=1) +
...
# Plotting Bollinger Bands with Tidyquant

```{r}
funds |> filter(date >="2024-01-01") |> ggplot(aes(x=date,y=close)) + 
        geom_line() +
  geom_bbands(aes(high = high, low = low, close = close), ma_fun = SMA, n = 20) +
  labs(title="Vanguard Dividend Appreciation Index Fund(VDADX)",subtitle="(Bollinger Bands using 20 Day Moving Average)") +
...
bollinger_bands <- funds |> filter(date >='2024-01-01') %>%
...
```{r}
funds %>%  filter(date >='2024-01-01') |> ggplot() +
      geom_line(aes(x=date,y=close,col="Close")) +
  geom_line(aes(x=date,y=adjusted, col="Adjusted"))

```
# Create A Stock Barchart the Hardway

# ```{r}
# barchart <- funds |> select(date,open,high,low,close) |>
#   pivot_longer(
#     cols = c(open, high, low, close),  # Columns to pivot
#     names_to = "price_type",           # New column for price type
